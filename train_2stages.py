import os
from dataset_2stages import Dataset_stage1, Dataset_stage2
...
from Network import Full_Resolution_Net
...
parser.add_argument('--rcloss', type=int, default=0, help='rcloss')
...
if args.name == 'stage1':
    ...
    save_dir = os.path.join('trained_models/{}_rcloss{}'.format(args.name, args.rcloss), save_name)
    ...
    input_size = (352, 448)
    train_data = Dataset_stage1(data_root=data_dir, split_path=split_path, fold=0, mode='train', input_size=input_size)
    val_data = Dataset_stage1(data_root=data_dir, split_path=split_path, fold=0, mode='val', input_size=input_size)
    net = smp.Unet(in_channels=1, classes=1, encoder_name='resnet34').cuda()
elif args.name == 'stage2':
    save_name = 'bs{}_epoch{}_fold{}_seed{}'.format(args.bs, args.epoch, args.fold, args.seed)
    save_dir = os.path.join('trained_models/{}_rcloss{}_size{}'.format(args.name, args.rcloss, args.stage2_size), save_name)
    if os.path.exists(save_dir):
        shutil.rmtree(save_dir)
    os.makedirs(save_dir, exist_ok=True)
    train_writer = SummaryWriter(os.path.join(save_dir, 'log/train'), flush_secs=2)
    ...
    net = Full_Resolution_Net(in_channel=1, classes=1).cuda()
...
scheduler = MultiStepLR(optimizer, milestones=[int((6 / 10) * args.epoch),
...
for epoch in range(args.epoch):
    ...
    for i, (img, label) in enumerate(train_dataloader):
        ...
        if args.name == 'stage1' and args.rcloss == 1:
            loss_Seg = BCEloss + diceloss + 10 * rcloss
        elif args.name == 'stage2' and args.rcloss == 1:
            loss_Seg = BCEloss + diceloss + rcloss
        elif args.rcloss == 0:
            loss_Seg = BCEloss + diceloss
        loss_Seg.backward()
