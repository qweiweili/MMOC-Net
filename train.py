import os
from dataset import Dataset_CMB
...
parser.add_argument('--name', type=str, default='Syn_first', help='net name')
...
save_dir = os.path.join('trained_models/{}_{}'.format(args.name, args.net), save_name)
...
train_data = Dataset_CMB(data_root=data_dir, split_path=split_path, fold=0, mode='train', input_size=input_size)
train_dataloader = DataLoader(dataset=train_data, batch_size=args.bs, shuffle=True, num_workers=2, pin_memory=True)
val_data = Dataset_CMB(data_root=data_dir, split_path=split_path, fold=0, mode='val', input_size=input_size)
...
for epoch in range(args.epoch):
    ...
    for i, (img, label) in enumerate(train_dataloader):
        ...
        loss_Seg = BCEloss + diceloss + 5 * rcloss
        loss_Seg.backward()
    ...
    with torch.no_grad():
        for i, (img, label) in enumerate(val_dataloader):
            ...
            for b in range(pred.size(0)):
                epoch_val_precision.append(precision_score(label_array[b], predictions[b]))
                epoch_val_recall.append(recall_score(label_array[b], predictions[b]))
